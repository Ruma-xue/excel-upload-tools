...
public class IndexController {
    ...
    public void download(HttpServletResponse response) throws Exception {
    	ProdectServer prodectServer = new ProdectServer();
        ExcelData data = new ExcelData();
        List<List<Object>> rows = new ArrayList();
        List<Object> row = new ArrayList();
        Class<? extends ProdectServer> aClass = new ProdectServer().getClass();
        Field[] declaredFields = aClass.getDeclaredFields();
        List<String> titles = new ArrayList<>();
        for (Field f : declaredFields) {
            titles.add(f.getName());
            f.setAccessible(true);
            row.add(f.get(prodectServer));
        }
        data.setTitles(titles);
        rows.add(row);
        data.setRows(rows);
        data.setName("prodectServer");
        ExportExcelUtils.exportExcel(response, "prodectServer.xlsx", data);
    }
